    ...
    return (
        <section className="py-20 bg-white/40 border-t border-white/50" id="reviews">
            <div className="max-w-[1200px] mx-auto px-4">
                <h2 className="text-center text-4xl font-serif font-bold text-text-main mb-16">Love Stories from Rosella</h2>
                    ...
                    {reviews.map((review, index) => (
                        <div key={index} className="bg-white p-8 rounded-[2rem] relative shadow-[0_10px_40px_-10px_rgba(219,39,119,0.1)] hover:translate-y-[-5px] transition-transform duration-300 border border-secondary/5">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 className="font-bold text-accent-maroon font-serif text-lg">{review.name}</h4>
                                    <p className="text-xs text-secondary uppercase tracking-wider font-semibold">{review.detail}</p>
                                </div>
                            </div>
                        </div>
                    ))}
